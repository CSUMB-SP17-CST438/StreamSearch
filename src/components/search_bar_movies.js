...
class SearchBar extends Component {
	...
	render() {
		return (
			...
			<div>
				<div>
				<Link key='1' to={'/shows'} onClick={() => this.setShow()}>
						<h2 className="search-key">Show</h2>
				</Link>
				<Link key='2' to={'/movies'} onClick={() => this.setMovie()}>
	...
	onInputChange(value) {
		...
		if (this.state.select == "movies")
			this.state.sendMovieQuery(value);
		else
			this.state.sendShowQuery(value);
		Socket.emit('search1', {
                'text': value
            });
	}
...
export default connect(mapStateToProps, { searchMovies, searchShows })(SearchBar);